...
class GuardiaForm extends StatefulWidget {
  final String? idUsuario;

  const GuardiaForm({super.key, this.idUsuario});

  @override
  _GuardiaFormState createState() => _GuardiaFormState();
}

class _GuardiaFormState extends State<GuardiaForm> {
  // Controladores
  ...
  void initState() {
    ...
    turnoController = TextEditingController();
  ...
  Future<void> _fetchUsuario() async {
    ...
    final url = Uri.parse("http://10.0.2.2:8000/guardias/${widget.idUsuario}/");
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          turnoController.text = data["turno"] ?? '';
        });
      } else {
  ...
  Future<void> _guardarUsuario() async {
    final url = widget.idUsuario == null
        ? Uri.parse('http://10.0.2.2:8000/guardias/')
        : Uri.parse('http://10.0.2.2:8000/guardias/${widget.idUsuario}/');
